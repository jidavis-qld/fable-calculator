...
|---|---|---|---|---|
| D2 | **Accept a `gitleaks` baseline so the scan can go green?** The job has failed on **every** `main` run since 2026-06-04, because it scans full history (`--log-opts="--all"`) with `--exit-code 1` and no `--baseline-path`. A permanently-red check hides any genuinely new leak. `claude-plugins` already solved this (`gitleaks.yml:31` plus a committed 12-finding baseline) and is green. But committing a baseline means *accepting* what is in it, so the real decision is to look at the historical findings and rule on each: rotate, purge, or baseline. That review needs you — it may involve live credentials. | Fable's secret scanning stays permanently red here, and a new leak looks identical to the old ones | `docs/backlog/BACKLOG.md` [L3] | 2026-08-10 |
